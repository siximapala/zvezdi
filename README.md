...
Сетевой режим работает через встроенный WebSocket-сервер в `npm run dev`.

Для игры на одном компьютере открой несколько вкладок с адресом dev-сервера. Для игры по локальной сети запусти сервер так, чтобы он слушал внешний интерфейс:

```powershell
$env:HOST="0.0.0.0"; npm run dev
```

После этого другие игроки открывают адрес компьютера-хоста, например:
...
```text
Клик по цветной звезде - выбрать свободного персонажа; если лобби ещё нет, оно создастся и сразу запустит уровень
Код лобби - показывается на уровне, клик по коду копирует его
J / Войти по коду - подключиться к существующему лобби
K - отключиться от лобби
Enter - перейти дальше после прохождения уровня
```

Хост считает физику и рассылает состояние игры. В лобби можно играть одному, вдвоём или втроём. Один персонаж не может быть занят двумя игроками; занятые звёзды затемняются и не выбираются. Свободные персонажи остаются у хоста. Если все вышли, пустое лобби хранится 10 минут и удаляется только если никто не вернулся.

В сетевой игре каждый игрок управляет выбранной звездой одинаково:
