...
Если шипы или цветной блок случайно остались на `Neutral`, но у объекта есть `material`/`shape`, игра всё равно распознает его как material-объект. Для читаемости уровня лучше держать такие объекты на `Materials`.
...
Мята цепляется клавишей `O`, если anchor не за стеной.
